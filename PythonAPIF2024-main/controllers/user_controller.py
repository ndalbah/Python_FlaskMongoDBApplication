...
def generate_hash_password(password):
    ...
    return hashed_password  # Keep it as bytes, don't decode to utf-8
...
def login_user(user_information):
    try:
        ...
        jwt_data = {'email': current_user['email'], 'id': str(current_user['_id']), 'name': current_user['name'],'exp': expiration}

        jwt_to_return = jwt.encode(payload = jwt_data, key = config.TOKEN_SECRET, algorithm = "HS256")
